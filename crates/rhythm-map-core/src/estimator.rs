...
impl TempoMapEstimator {
    ...
    pub fn estimate(&self, input: &RhythmObservations) -> Result<Analysis, AnalysisError> {
        ...
        add_ramp_boundaries(&tempo_segments, &mut change_points);
        add_discontinuities(
            input,
            &intervals,
        ...
        let rhythm_sections = build_sections(input, &tempo_curve, &change_points);
...
fn add_ramp_boundaries(segments: &[TempoSegment], changes: &mut Vec<ChangePoint>) {
    for pair in segments.windows(2) {
        let significant_ramp = pair.iter().any(|segment| {
            segment.kind == TempoSegmentKind::Ramp
                && segment.end_s - segment.start_s >= 4.0
                && (segment.end_bpm / segment.start_bpm).ln().abs() >= 0.05_f64.ln_1p()
        });
        if pair[0].kind != pair[1].kind && significant_ramp {
            let time_s = pair[0].end_s;
            if changes
                .iter()
                .all(|change| (change.time_s - time_s).abs() > 0.5)
            {
                changes.push(ChangePoint {
                    time_s,
                    kind: ChangeKind::RampBoundary,
                    score: pair[0].confidence.min(pair[1].confidence),
                    before_bpm: Some(pair[0].end_bpm),
                    after_bpm: Some(pair[1].start_bpm),
                });
            }
        }
    }
...
mod tests {
    ...
    fn constant_tempo_stays_constant() {
        let input = observations_from_bpms(&vec![120.0; 32]);
