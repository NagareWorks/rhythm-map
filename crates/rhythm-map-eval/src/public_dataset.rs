...
fn download_zip_member(
    ...
    let written = match entry.compression_method {
        0 => {
            ...
            io::copy(&mut Cursor::new(compressed).take(output_limit), &mut output)
        }
        8 => io::copy(
...
fn fetch_zip_central_directory(agent: &Agent, url: &str, archive_size: u64) -> Result<Vec<u8>> {
    ...
    let directory_end = checked_range_end(directory.offset, directory.size, "central directory")?;
    request_range(agent, url, directory.offset, directory_end)
}
...
fn fetch_zip_directory(agent: &Agent, url: &str, archive_size: u64) -> Result<ZipDirectory> {
    ...
    let tail = request_range(agent, url, tail_offset, archive_size - 1)?;
    let eocd = tail
        .windows(4)
    ...
    let zip64 = request_range(agent, url, zip64_offset, zip64_end)?;
