//! Exact, evaluation-only semi-Markov clock search. No truth or peak list input.
...
/// Exact sum-product AND max-product L1 transform on log-period coordinates.
/// Excluding the center from the right sweep avoids counting its mass twice.
fn transitions(input: &[Node], coordinates: &[f64], norms: &[f64], output: &mut [Node]) {
    let mut carry = Node::EMPTY;
    for j in 0..input.len() {
        if j > 0 {
            carry = carry.shift(coordinates[j - 1] - coordinates[j], 0.);
        }
        carry.merge(input[j].shift(-norms[j], 0.));
        output[j] = carry;
    }
    carry = Node::EMPTY;
...
fn search(beat: &[f32], bar: &[f32], offset: usize, domain: Domain) -> Run {
    ...
    let coordinates: Vec<f64> = (domain.min_period..=domain.max_period)
        .map(|p| 100_f64.ln() * (p as f64).log2())
        .collect();
    let norms: Vec<f64> = coordinates
        .iter()
        .map(|x| {
            coordinates
                .iter()
    ...
    for start in 0..=n - domain.min_period * domain.min_meter {
        ...
        transitions(&source, &coordinates, &norms, &mut transformed);
        ...
        for beats in 1..=domain.max_meter {
            ...
            for elapsed in min_elapsed..=max_elapsed {
                ...
                transitions(row, &coordinates, &norms, &mut transformed);
    ...
    for end in n.saturating_sub(max_bar - 1)..=n {
        for p in 0..count {
            ...
            let candidate = global[index].shift(tail_prior, 0.);
...
mod tests {
        ...
        fn beats(
            ...
            if remaining == 0 {
                ...
                if self.beat.len() - cursor < max_bar {
                    self.total.merge(finished.shift(-(max_bar as f64).ln(), 0.));
                }
            ...
            for duration in self.domain.min_period..=self.domain.max_period {
                ...
                let prior = previous.map_or_else(
                    ...
                    |p| {
                        let penalty =
                            |q: usize| -100_f64.ln() * ((p as f64 / q as f64).log2()).abs();
                        penalty(duration)
                            - (self.domain.min_period..=self.domain.max_period)
                                .map(penalty)
                                .fold(NEG, log_add)
                    },
                );
    ...
    fn both_partitions_and_map_match_exhaustive_unknown_path_enumeration() {
        ...
        for tick in &run.map_ticks {
            ...
            reconstructed += previous.map_or_else(
                ...
                |p: usize| {
                    let penalty = |q: usize| -100_f64.ln() * (p as f64 / q as f64).log2().abs();
                    penalty(duration)
                        - (domain.min_period..=domain.max_period)
                            .map(penalty)
                            .fold(NEG, log_add)
                },
            );
    ...
    fn optimized_transform_matches_all_pairs_without_double_counting() {
        let x: Vec<f64> = vec![0., 0.2, 1.7, 2.1];
        let norms: Vec<f64> = x
            .iter()
            .map(|a| x.iter().map(|b| -(a - b).abs()).fold(NEG, log_add))
            .collect();
        let source: Vec<Node> = [0.3, -4., 1.2, -2.]
            ...
            .collect();
        let mut fast = vec![Node::EMPTY; x.len()];
        transitions(&source, &x, &norms, &mut fast);
        for j in 0..x.len() {
            let mut slow = Node::EMPTY;
            for i in 0..x.len() {
                slow.merge(source[i].shift(-(x[i] - x[j]).abs() - norms[i], 0.));
            }
            assert!((fast[j].mass - slow.mass).abs() < 1e-12);
            assert!((fast[j].reference - slow.reference).abs() < 1e-12);
            assert!((fast[j].best - slow.best).abs() < 1e-12);
            assert_eq!(fast[j].origin, slow.origin);
        }
    }
