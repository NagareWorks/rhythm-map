//! Truth-free joint clock reference: authored controls, not production inference.
use anyhow::Result;
use serde_json::json;
use sha2::{Digest, Sha256};
#[path = "support/joint_clock.rs"]
mod clock;
#[cfg(test)]
...
fn audit() -> Result<serde_json::Value> {
    ...
    for (name, periods, weak, alternating) in [
        ...
        eprintln!("joint clock authored control: {name}");
    ...
    for name in [
        ...
        eprintln!("joint clock boundary control: {name}");
        let started = std::time::Instant::now();
    ...
    Ok(
        json!({"schema_version":1,"purpose":"exact_joint_clock_authored_gate",
        ...
        "decoder_source_sha256":format!("{:x}",Sha256::digest(include_bytes!("support/joint_clock.rs"))),
        "audit_source_sha256":format!("{:x}",Sha256::digest(include_bytes!("joint_clock.rs")))}),
    )
}
