//! Common-context meter audit on frozen inferred ticks; evaluation only.
use anyhow::Result;
use serde_json::json;
use sha2::{Digest, Sha256};
#[path = "support/common_meter.rs"]
mod clock;

fn mark(values: &[f32]) -> f64 {
    0.25 * f64::from(values[values.len() - 1])
        + 0.5 * f64::from(values[0])
        + 0.25 * f64::from(values[1])
}
...
fn audit() -> Result<serde_json::Value> {
    ...
    Ok(
        json!({"schema_version":1,"purpose":"common_context_conditional_meter_gate",
        ...
        "decoder_source_sha256":format!("{:x}",Sha256::digest(include_bytes!("support/common_meter.rs"))),
        "previous_report_sha256":format!("{:x}",Sha256::digest(include_bytes!("../../../evaluation/parity/censored-meter-v1.json"))),
        "audit_source_sha256":format!("{:x}",Sha256::digest(include_bytes!("common_meter.rs")))}),
    )
}
...
fn conditional(name: &str, beat: &[f32], bar: &[f32]) -> Result<serde_json::Value> {
    ...
    for run in row["decoded"]["runs"].as_array().unwrap() {
        ...
        for tick in ticks {
            ...
            marks.push(mark(&bar[start..start + duration]));
        }
        let inference = clock::infer(&marks, 2, 7)?;
        runs.push(json!({"frozen_ticks":ticks,"mark_scores":marks,"meter":inference,
        ...
        for phase in 0..meter {
            ...
            for removed in 0..meter {
                ...
                rows.push(
                    ...
                    "visible_beats":length,"mark_scores":&marks[..length],"inference":inference}),
                );
            }
        }
    }
    ...
    for (name, first, second) in [
        ...
        for meter in [first, second] {
            for i in 0..24 {
                ...
                marks.push(mark(&cell));
                meters.push(meter);
            }
        }
        let inference = clock::infer(&marks, 2, 7)?;
        rows.push(json!({"case":name,"authored_meters":meters,
            "mark_scores":marks,"inference":inference}));
    }
    Ok(rows)
}
