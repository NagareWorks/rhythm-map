...
from experiments.coupled_clock.measurement import macro, KEYS
...
from experiments.protected_phase_fit.register import ROOT, sha, write_json, make_plan
from experiments.protected_phase_fit.recorder import MusicalRecorder, score, conditioning
...
def execution_gate(fits, plan):
    return len(fits) == 2 and {row['kind'] for row in fits} == {'audio', 'zero-audio'} and all(
        ...
        and row['conditioning']['complete_records'] == plan['complete_records']
        and row['conditioning']['phase_gradients'] == plan['complete_records']
        and row['conditioning']['count_gradients'] == plan['complete_records']
        and row['conditioning']['protected_steps'] == plan['updates']
        and row['conditioning']['post_step_records'] == plan['complete_records']
        and row['conditioning']['committed_steps'] == plan['updates']
        and row['conditioning']['adverse_defined_groups'] == 0
        and row['conditioning']['group_receipts'] == 3 * plan['updates']
        and row['conditioning']['moved_steps'] + row['conditioning']['stalled_steps'] == plan['updates']
...
def compare_previous(rows, summary):
    """Descriptive mechanism comparison, never a replacement acceptance gate."""
    from experiments.phase_sync_fit.run import same_support
    previous = json.loads((ROOT / 'experiments/scaled_phase_fit/results-v1.json').read_bytes())
    require(len(rows) == len(previous['cases']), 'previous comparison population differs')
    cases = []
    for row, old in zip(rows, previous['cases']):
        require(all(row[k] == old[k] for k in ('id', 'role', 'work', 'frames', 'packet_sha256')),
                'previous comparison case differs')
        same_support(row, old)
        delta = {k: (None if row['audio_common'][k] is None or old['audio_common'][k] is None else
                     row['audio_common'][k] - old['audio_common'][k]) for k in KEYS}
        cases.append(dict(id=row['id'], role=row['role'], old_audio_common=old['audio_common'], delta=delta))
    cohorts = {role: {k: dict(previous=previous['summary'][role]['audio_common'][k],
                              current=summary[role]['audio_common'][k])
                     for k in KEYS} for role in ('fit', 'development', 'diagnostic')}
    return dict(report_sha256=sha(ROOT / 'experiments/scaled_phase_fit/results-v1.json'),
                cases=cases, cohorts=cohorts, changes_accuracy_gate=False)
...
def main():
    ...
    args.output.mkdir()
    write_json(args.output / 'execution.json', dict(schema='rhythm-map.protected-phase-execution.v1',
    ...
    try:
        ...
        previous_comparison = compare_previous(rows, summary)
        gate = comparison_gate(rows, summary, fits)
        gate['protected_execution_complete'] = execution_gate(fits, plan)
        ...
        report = dict(schema='rhythm-map.protected-phase-learning.v1',
            ...
            cases=rows, summary=summary, gate=gate, previous_scaled_comparison=previous_comparison,
            training=True, optimizer_fits=2,
